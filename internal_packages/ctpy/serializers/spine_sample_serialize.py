...
from ..samples import SpineSample
from ..volume import Volume


class SpineSampleSerialization:
    """
    ...
    def from_volume(
        ...
        """
        ras_vol = ct_volume.get_warped(space='RAS')
        ct = ras_vol.pixel_data
        ras_vol = mask_volume.get_warped(space='RAS')
        mask = ras_vol.pixel_data
        voxel_size = ras_vol.voxel_size
        sample = SpineSample(ct, mask, voxel_size, name)
        return sample

    def from_h5(self, ds: h5py.File, i: int) -> SpineSample:
        """
    ...
    def to_h5(self, ct_sample: SpineSample,
        ...
        kwargs = {
            ...
            'mask': np.reshape(ct_sample.mask, (-1,)),
        ...
        if isinstance(manager, h5py.File):
            ...
            raise RuntimeError(f'Unknown manager type for SpineSample dump '
                               f'into h5: {manager=}.')
