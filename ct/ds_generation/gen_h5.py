...
from ctpy.serializers import CTSampleSerialization
...
def parse_args():
    ...
    inputs.add_argument(
        '--ds-root', type=Path,
        help='Dataset root to generate CTSample instances from. '
...
def main():
    ...
    dataset = factory.create_dataset(args.ds_root)

    serialize_manager = CTSampleSerialization(
    ...
    for split in ['train', 'test', 'val']:
        ...
        with H5DSCreationManager(
            ...
            ct_sample_gen = dataset.get_ctsample_gen(split)
            progress_bar = tqdm(dataset.__getattribute__(f'{split}_names'))
            progress_bar.set_description(f'Generating {split}...')
            for ct_sample in ct_sample_gen:
                serialize_manager.to_h5(ct_sample, h5_manager)
